...
def rewrite_for_reretrieval(query: str, iteration: int) -> str:
    ...
    if iteration == 2:
        ...
        m = re.search(r"(《?[\u4e00-\u9fff]{2,15}法》?)\s*第\s*([一二三四五六七八九十百千零0-9]+)\s*条", query)
        if m:
            law = m.group(1)
            num = m.group(2)
        ...
        law_match = re.search(r"(民法典|刑法|劳动合同法|劳动法|个人信息保护法|消费者权益保护法|公司法|合同法|著作权法|商标法|专利法|道路交通安全法|电子商务法|网络安全法|数据安全法|行政诉讼法|民事诉讼法|刑事诉讼法|监察法|宪法)", query)
        if law_match:
