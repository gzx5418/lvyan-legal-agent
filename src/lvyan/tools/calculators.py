...
_EVIDENCE_CHECKLIST: dict[str, list[tuple[str, str, str]]] = {
    ...
    # P1 补缺：triage 的标准案由「合同纠纷 / 婚姻家庭 / 知识产权」此前无对应
    # 清单，evidence_analyzer 静默返回空（三大案由完全没有证据缺口分析）。
    "合同纠纷": [
        ("合同文本（原件或完整截图）", "证明合同关系成立及权利义务约定", "required"),
        ("履行凭证（交付记录、物流单据、验收单）", "证明己方已按约履行", "required"),
        ("付款/收款凭证（转账记录、发票、收据）", "证明金钱给付事实与金额", "required"),
        ("违约事实证据（催告函、聊天记录、拒收记录）", "证明对方违约及己方已催告", "required"),
        ("损失证明（额外采购合同、停工损失、评估报告）", "支持违约损失赔偿请求", "recommended"),
        ("双方沟通记录（微信/邮件/会议纪要）", "证明合同变更、解除或免责约定", "recommended"),
    ],
    "婚姻家庭": [
        ("结婚证/婚姻登记记录", "证明婚姻关系", "required"),
        ("子女出生证明/户口簿", "确定抚养权与抚养费主体", "required"),
        ("财产凭证（房产证、车辆登记、存款/理财对账单）", "划分夫妻共同财产", "required"),
        ("感情破裂证据（分居记录、报警记录、保证书、聊天记录）", "证明感情确已破裂", "recommended"),
        (
            "家暴/虐待证据（报警回执、验伤报告、告诫书、人身安全保护令）",
            "支持损害赔偿与保护措施",
            "recommended",
        ),
        ("收入证明（工资流水、纳税记录）", "计算抚养费/赡养费", "recommended"),
    ],
    "知识产权": [
        (
            "权属证明（著作权登记证书、专利证书、商标注册证、创作底稿）",
            "证明权利归属与效力",
            "required",
        ),
        (
            "侵权行为证据（侵权页面公证/时间戳存证、实物、销售记录）",
            "证明侵权事实与范围",
            "required",
        ),
        ("权利在先证据（发表时间、首次公开记录）", "证明权利形成早于被诉行为", "required"),
        ("许可合同或授权文件", "证明许可使用范围与维权资格", "recommended"),
        ("损失或获利证据（销量、定价、财务报表、许可费参照）", "支持赔偿数额主张", "recommended"),
        ("维权合理开支票据（公证费、律师费、取证费）", "主张合理开支", "recommended"),
    ],
}
"""案类型 -> 证据清单映射。"""
