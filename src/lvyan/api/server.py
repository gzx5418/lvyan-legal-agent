...
def create_app(
    ...
    Args:
        ...
        workspace_store: 可注入的案件工作空间仓库；生产 PostgreSQL 自动持久化。
    """
    ...
    if workspace_store is None:
        ...
        # 保持进程内存储，避免未配置数据库时静默写入其他位置。
        if isinstance(metadata_store, PostgresRunMetadataStore):
            workspace_store = PostgresCaseWorkspaceStore()
        else:
    ...
    @app.post("/api/cases", response_model=LegalCase, status_code=201)
    ...
    @app.get("/api/cases/{case_id}/evidence", response_model=list[CaseEvidence])
    ...
    async def link_case_evidence(
        ...
        meta_path = _metadata_path_for_file_id(req.file_id)
    ...
    @app.get("/api/cases/{case_id}/documents", response_model=list[LegalDocument])
    ...
    @app.get("/api/legal-documents/{document_id}", response_model=LegalDocument)
    ...
    @app.patch("/api/review-findings/{finding_id}", response_model=ReviewFinding)
