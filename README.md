...
在网页“设置 → 通用”中启用“联网检索权威来源”后，系统会将**脱敏且截断的检索词**发送给 Bing 进行发现，并只展示政府、人大、法院和检察院官网的 HTTPS 链接。该功能默认关闭；结果以“供核对”单列展示，不会替代本地法律库中经过版本与引用校验的法条。部署方可设置 `WEB_SEARCH_ENABLED=false` 完全禁用外网检索。
